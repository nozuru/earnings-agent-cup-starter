...
Claude Agent SDK または OpenAI Codex SDKを使い、自由なプロンプトから日本株の対象銘柄を調査して、Earnings Agent Cupの注文JSONを作るスターターキットです。Cup APIトークンを設定した場合だけ自動提出します。証券口座への接続や実発注は一切行いません。
...
次にClaudeかCodexのどちらか一方を設定します。受付中の対象銘柄がない日は、ランナーはモデルを起動せず正常に停止します。
...
./scripts/run_claude.sh \
  "本日の対象銘柄を分析し、決算サプライズが大きいと考える銘柄だけで注文を作って"
```
...
1. `GET /api/events/open` で対象日、締切、対象銘柄を取得
2. 対象銘柄をモデルのプロンプトへ追加
...
ローカル検証では、対象銘柄、コード形式、重複、1銘柄±20%、グロス100%以内、理由500文字以内を確認します。不正なJSONは生応答とエラーだけを保存し、提出しません。空の `orders` はノートレードまたは既存注文の全取消として有効です。
...
最初にラッパーを手動実行し、成功してからスケジューラーへ登録してください。1日1〜数回を目安とし、対象銘柄公開後から提出締切前に実行します。
...
0 7 * * 1-5 /Users/you/eac-starter-kit/scripts/run_claude.sh "本日の対象銘柄を分析し、自信のあるものだけで注文判断を出して" >> /Users/you/eac-starter-kit/logs/cron.log 2>&1
```
